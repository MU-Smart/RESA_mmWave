"""Curated Branch 1 rolling retraining driver."""
...
DEFAULT_EXTERNAL_VAL_QUALITY_TIERS = ("gold",)
...
class Config:
    ...
    dry_run: bool
    use_oneformer_batches: bool
    oneformer_batch_ids: tuple[str, ...]
    oneformer_val_batch_id: str
    oneformer_excluded_batch_ids: tuple[str, ...]
    ...
    @property
    def gcs_oneformer_output_root(self) -> str:
        return f"{self.gcs_root_uri}/intermediate/oneformer/outputs"
...
def resolve_paths(cfg: Config) -> Paths:
    ...
    return Paths(
        ...
        train_out=cfg.work_root / "_train_output" / "kpconv_branch1_4class_depth_directness_curated_true_ra_v1",
    )
...
def session_records(cfg: Config, paths: Paths) -> list[tuple[str, str, str]]:
    ...
    if not records:
        raise RuntimeError(
            "No curated training sessions matched the current catalog filters. "
...
def build_external_validation_dataset(cfg: Config, paths: Paths) -> Path:
    ...
    try:
        ...
        combined_source = build_combined_batch_dataset(cfg, paths, 0, source_dirs, preserve_splits=False)
        if val_root.exists():
        ...
        for root in group_to_root.values():
            if root.exists():
                shutil.rmtree(root)
    print("Built curated external validation dataset:", val_root, flush=True)
    return val_root


def build_combined_batch_dataset(cfg: Config, paths: Paths, batch_id: int, source_dirs: list[tuple[str, Path]], *, preserve_splits: bool = False) -> Path:
...
def train_one_batch(cfg: Config, paths: Paths, batch_id: int, records: list[tuple[str, str, str]], external_val_root: Path | None) -> None:
    """Train one rolling batch.

    When external_val_root is None the batch is built with an internal
    train/val holdout (last session per group held out) and the training
    script uses the internal val split for metric tracking.
    When external_val_root is a Path the dataset is all-train and the
    training script uses the external val set.
    """
    use_internal_val = external_val_root is None
    holdout_last = 1 if use_internal_val else 0
    ...
    batch_root = build_combined_batch_dataset(
        cfg, paths, batch_id, source_dirs, preserve_splits=use_internal_val
    )
    debug_dataset_csv(batch_root, f"training batch {batch_id}")
    ...
    if use_internal_val:
        cmd.extend(["--best-metric", "val_bal_acc"])
    else:
        cmd.extend([
...
def build_oneformer_validation_dataset(cfg: Config, paths: Paths) -> Path:
    """Build external validation dataset from a dedicated oneformer batch.

    Uses cfg.oneformer_val_batch_id as the source batch, taking up to
    cfg.validation_session_count sessions.  This batch is excluded from
    the training sweep in run_rolling_training.
    """
    val_root = paths.rolling_val_root / "rdra_patch_dataset"
    val_csv = val_root / "points_with_rd_patch_index.csv"
    bid = cfg.oneformer_val_batch_id
    if val_csv.exists() and _dataset_has_ghost_inputs(val_root) and not cfg.force_rebuild_external_val:
        print("Reusing cached oneformer validation dataset:", val_csv, flush=True)
        force_dataset_split(val_root, "val")
        debug_dataset_csv(val_root, f"oneformer validation reuse (batch_{bid})")
        return val_root
    if paths.rolling_val_root.exists():
        shutil.rmtree(paths.rolling_val_root)

    records = oneformer_batch_session_records(cfg, paths, bid)
    if not records:
        raise RuntimeError(
            f"No sessions found in oneformer batch_{bid} for validation. "
            f"Looked under: {cfg.gcs_oneformer_output_root}/batch_{bid}/"
        )
    if cfg.validation_session_count > 0:
        records = records[: cfg.validation_session_count]

    group_label = records[0][0]  # == cfg.dataset_groups[0]
    val_processing_root = paths.rolling_val_root / "processing" / group_label
    val_processing_root.mkdir(parents=True, exist_ok=True)
    print(
        f"Downloading {len(records)} validation session(s) from batch_{bid}.",
        flush=True,
    )
    _download_records_to_roots(
        cfg, paths, records, {group_label: val_processing_root}
    )

    old_dataset_root = paths.rolling_dataset_root
    paths.rolling_dataset_root = paths.rolling_val_root / "datasets"
    try:
        val_source = build_source_dataset(
            cfg,
            paths,
            f"batch_{bid}_val",
            val_processing_root,
            cfg.session_prefix,
            0,
            dataset_tag=f"oneformer_batch_{bid}_val",
            skip_validate=True,
        )
        if val_root.exists():
            shutil.rmtree(val_root)
        shutil.move(str(val_source), str(val_root))
        force_dataset_split(val_root, "val")
        debug_dataset_csv(val_root, f"oneformer validation built (batch_{bid})")
    finally:
        paths.rolling_dataset_root = old_dataset_root
        if val_processing_root.exists():
            shutil.rmtree(val_processing_root)
    print("Built oneformer validation dataset:", val_root, flush=True)
    return val_root


def _extract_sessions_from_ls(
    matches: list[str],
    base: str,
    session_prefix: str,
    group_label: str,
    ...
    """Parse gcloud storage ls results into (group_label, name, uri) records.

    depth=0  expects URIs like  base/session_NAME/...
    depth=1  expects URIs like  base/SUBDIR/session_NAME/...
    """
    records: list[tuple[str, str, str]] = []
    for uri in matches:
        rel = uri.rstrip("/")[len(base):].lstrip("/")
        parts = rel.split("/")
        if len(parts) <= depth:
            continue
        name = parts[depth]
        if not name or not name.startswith(session_prefix) or name in seen:
            continue
        seen.add(name)
        # Reconstruct the canonical session URI (no trailing file components).
        sess_uri = base + "/" + "/".join(parts[: depth + 1])
        records.append((group_label, name, sess_uri))
    return records


def oneformer_batch_session_records(
    cfg: Config,
    paths: Paths,
    bid: str,
) -> list[tuple[str, str, str]]:
    """List (group_label, session_name, gcs_uri) for sessions in
    gcs_oneformer_output_root/batch_{bid}/.

    Performs a two-tier search:
      Tier 1 — sessions directly under batch_XXXX/session_*
      Tier 2 — sessions under one intermediate subdir  batch_XXXX/{subdir}/session_*
               (the preprocessing pipeline groups sessions by raw-data group name)
    Whichever tier finds sessions first is used; results from both tiers are
    combined when both return hits.
    cfg.dataset_groups[0] is used as the synthetic group label so the
    download/training pipeline routes sessions correctly.
    bid should be zero-padded (e.g. "0001").
    """
    if not cfg.dataset_groups:
        raise RuntimeError("cfg.dataset_groups must contain at least one group name.")
    group_label = cfg.dataset_groups[0]
    batch_key = f"batch_{bid}"
    base = f"{cfg.gcs_oneformer_output_root}/{batch_key}"
    prefix = cfg.session_prefix
    seen: set[str] = set()
    records: list[tuple[str, str, str]] = []

    # ── Tier 1: sessions directly under batch_XXXX/ ──────────────────────
    tier1 = gcloud_storage_ls(
        f"{base}/{prefix}*",
        cwd=paths.root,
        env=paths.env,
        dry_run=cfg.dry_run,
    ...
    records.extend(_extract_sessions_from_ls(tier1, base, prefix, group_label, seen, depth=0))

    # ── Tier 2: sessions under batch_XXXX/{subdir}/ (group layer) ────────
    # The rigorous-preprocess pipeline stores sessions under a raw-data group
    # subfolder whose name is not known in advance, so we search one level
    # deeper with a wildcard.
    tier2 = gcloud_storage_ls(
        f"{base}/*/{prefix}*",
        cwd=paths.root,
        env=paths.env,
        dry_run=cfg.dry_run,
    ...
    records.extend(_extract_sessions_from_ls(tier2, base, prefix, group_label, seen, depth=1))

    records.sort(key=lambda r: r[1])
    print(
        f"{batch_key}: found {len(records)} session(s) under {base}/"
        f" (tier1={len(tier1 or [])} raw hits, tier2={len(tier2 or [])} raw hits)",
        flush=True,
    )
    if records:
        print("  first:", records[:3], flush=True)
        print("  last :", records[-3:], flush=True)
    return records
...
def run_rolling_training(cfg: Config, paths: Paths, *, max_batches: int | None = None) -> None:
    ...
    print(
        f"Curated rolling training: {len(records)} train sessions in {len(batches)} batches of up to {cfg.session_batch_size}",
        flush=True,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--validation-session-count", type=int, default=5)
    ...
    # Oneformer-batch mode: train session-by-batch using the batch_XXXX subdirs produced
    # by the rigorous preprocessing pipeline under oneformer_outputs/.
    parser.add_argument("--use-oneformer-batches", action="store_true", help=argparse.SUPPRESS)
    parser.add_argument(
        "--oneformer-batch-ids",
        default="0001,0002,0003,0004,0005,0006,0007",
        help=(
            "Comma-separated zero-padded batch IDs to train over, e.g. "
            "\"0001,0002,0003\". Only used when --use-oneformer-batches is set."
        ),
    )
    parser.add_argument(
        "--oneformer-val-batch-id",
        default="0001",
        help=(
            "Zero-padded batch ID reserved for external validation "
            "(used by the separate ext-val cell). Default: \"0001\"."
        ),
    )
    parser.add_argument(
        "--oneformer-excluded-batch-ids",
        default="0005",
        help=(
            "Comma-separated batch IDs to skip during training, e.g. "
            "\"0005\" or \"0005,0006\". Default: \"0005\"."
        ),
    )
...
def make_config(args: argparse.Namespace) -> Config:
    ...
    return Config(
        ...
        use_oneformer_batches=args.use_oneformer_batches,
        oneformer_batch_ids=tuple(
            bid.strip()
            for bid in str(args.oneformer_batch_ids).split(",")
            if bid.strip()
        ),
        oneformer_val_batch_id=str(args.oneformer_val_batch_id).strip(),
        oneformer_excluded_batch_ids=tuple(
            bid.strip()
            for bid in str(args.oneformer_excluded_batch_ids).split(",")
            if bid.strip()
        ),
    )
