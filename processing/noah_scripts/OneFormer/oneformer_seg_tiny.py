...
def main():
    ...
    out_path = Path(args.out) if args.out else Path(str(base) + "_seg_tiny.mp4")

    # ---- Settings ----
    # Prefer local model folder inside repo/OneFormer if present.
    here = Path(__file__).resolve().parent
    local_model = here / "oneformer_tiny"
    default_model = str(local_model) if local_model.is_dir() else "shi-labs/oneformer_ade20k_swin_tiny"
