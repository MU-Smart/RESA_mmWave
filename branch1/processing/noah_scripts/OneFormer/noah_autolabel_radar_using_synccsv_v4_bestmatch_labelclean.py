...
What it does:
  ...
  - Runs the v4 best-match auto-labeling logic with cleaner ADE bucket mapping.
...
Optional:
        ...
        [--summary_csv batch_autolabel_summary_v4_labelclean.csv]
        [--stop_on_error]
...
def ade_terms(ade_name: str) -> list[str]:
    """Split an ADE label string like 'column, pillar' into normalized terms."""
    if not ade_name:
        return []
    terms = []
    for raw in str(ade_name).lower().split(","):
...
# Ignore decorative / signage classes that are visually prominent in the camera
# but are not useful navigation targets for sparse mmWave training.
IGNORE_ADE_TERMS = frozenset(
    {
        "signboard",
        "sign",
        "bulletin board",
...
def should_ignore_ade(ade_name: str) -> bool:
    ...
    if n in IGNORE_ADE_EXACT:
        return True
    terms = ade_terms(n)
    if any(term in IGNORE_ADE_TERMS for term in terms):
        return True
...
def map_bucket(ade_name: str) -> str:
    ...
    n = str(ade_name or "").lower().strip()
    terms = ade_terms(n)

    if "person" in n:
        return "human"

    # Use term-level matching so 'poster' no longer leaks into the pillar bucket
    # through the shorter substring 'post'.
    if any(term in {"column", "pillar", "pole", "post"} for term in terms):
        return "pillar"

    if "wall" in n or any(term == "partition" for term in terms):
        return "wall"

    if any(term in {"floor", "carpet", "rug", "mat"} for term in terms):
        return "floor"

    if "door" in n:
        return "door"

    if any(("stair" in term) or (term in {"escalator", "step"}) for term in terms):
        return "box_like"
...
def pick_radar_columns(df: pd.DataFrame) -> dict:
    ...
    is_radarhd_point = first_present(["is_radarhd_point"])
    radarhd_confidence = first_present(["radarhd_confidence"])
    ...
    return {
        ...
        "doppler": doppler,
        "snr": snr,
        "is_radarhd_point": is_radarhd_point,
        "radarhd_confidence": radarhd_confidence,
    }
...
def parse_semicolon_int_list(val) -> list[int]:
    ...
    if isinstance(val, (float, np.floating)):
        return [int(val)] if np.isfinite(val) else []
    s = str(val).strip()
    if not s:
        return []
    result = []
...
def process_one_session(
    ...
    roi_top_frac: float,
    out_csv: Path,
    assign_mode: str = "best_per_radar",
    ...
    if assign_mode == "best_per_radar":
        ...
        for rf, match in tqdm(sorted(best_map.items()), total=len(best_map), desc=processing_session_dir.name, leave=False):
            ...
            for i in range(len(pts)):
                ...
                if rcol["is_radarhd_point"] is not None:
                    rec["is_radarhd_point"] = float(grp.iloc[i][rcol["is_radarhd_point"]])
                if rcol["radarhd_confidence"] is not None:
                    rec["radarhd_confidence"] = float(grp.iloc[i][rcol["radarhd_confidence"]])
...
def main():
    ap = argparse.ArgumentParser(
        description="Batch auto-label radar points session by session using OneFormer segmentation maps (v4 label-clean)."
    )
    ap.add_argument(
        "--processing_root_dir",
        required=True,
    ...
    ap.add_argument(
        ...
        default="best_per_radar",
        help="How to consume sync_csv matches. 'best_per_radar' keeps one best video frame for each radar frame; 'all_within_window' preserves the older many-to-many behavior. Default: best_per_radar",
    )
    ap.add_argument(
        "--max_time_diff_ms",
        type=float,
    ...
    ap.add_argument(
        "--summary_csv",
        default=None,
        help="Optional batch summary CSV path. Default: <processing_root_dir>/batch_autolabel_summary_v4_labelclean.csv",
    )
    ap.add_argument(
        "--stop_on_error",
        action="store_true",
    ...
    summary_csv = (
        ...
        if args.summary_csv
        else processing_root_dir / "batch_autolabel_summary_v4_labelclean.csv"
    )
